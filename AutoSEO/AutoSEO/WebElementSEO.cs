...
namespace AutoSEO
{
    ...
    public class WebElementSEO
    {
        ...
        public WebElementSEO(DriverEnum driverEnum, string targetWebName)
        {
            ...
            logger.Info($"测试：x-forwarded-for={ip}");
        ...
        {
            int random = AppConstant.rd.Next(15) % 8;
            switch (random)
            {
                case 0:
        ...
        public void Jump(string keyWord, BrowserEnum browserEnum = BrowserEnum.baidu, string www = "https://www.baidu.com/", string inputId = "#kw", string btnId = "#su")
        {
            try
            {
                ...
                Thread.Sleep(TimeSpan.FromSeconds(AppConstant.rd.Next(3, 10)));
                ...
                Thread.Sleep(TimeSpan.FromSeconds(AppConstant.rd.Next(1, 3)));

                //3.搜索
                seleniumUtils.ClickByCss(btnId);
                Thread.Sleep(TimeSpan.FromSeconds(AppConstant.rd.Next(3, 5)));
                ...
                Thread.Sleep(TimeSpan.FromSeconds(AppConstant.rd.Next(5, 15)));
            ...
            //6.目标网址停留5-10分钟，关闭
            Thread.Sleep(TimeSpan.FromSeconds(AppConstant.rd.Next(AppConstant.startRequestTime, AppConstant.endRequestTime)));
            seleniumUtils.Close();
        }
        ...
        {
            int row = AppConstant.rd.Next(3);
            var arr = BitConverter.GetBytes(AppConstant.rd.Next(AppConstant.rangIP[row, 0], AppConstant.rangIP[row, 1]));
        ...
        private void RandomClickGT()
        {
            ...
            while (!result)
            {
                ...
                result = seleniumUtils.ClickByXpathForDisplayed("//*[@class='subtheme']/span/a[@href='/News/InstantNews']");
                Thread.Sleep(TimeSpan.FromSeconds(AppConstant.rd.Next(1, 3)));
                counter++;
            ...
            while (!result)
            {
                result = seleniumUtils.ClickByXpath($"//*[@id='maincontent']//div[contains(@class,'instantnews')]//li[{AppConstant.rd.Next(1, 7)}]/a[@class='tdclass']");
                counter++;
                if (counter > maxCounter) break;
            }

            //3.查看新闻
            seleniumUtils.ClickByXpath($"//*[@class='news_list'][{AppConstant.rd.Next(1, 20)}]/a");
            Thread.Sleep(TimeSpan.FromSeconds(AppConstant.rd.Next(3, 10)));
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                seleniumUtils.ClickByXpath($"//*[@class='news_list'][{AppConstant.rd.Next(1, 20)}]/a");
                Thread.Sleep(TimeSpan.FromSeconds(AppConstant.rd.Next(10, 20)));
            }
        }
        ...
        private void RandomClickMGT()
        {
            ...
            //3.查看新闻
            seleniumUtils.ClickByXpath($"//ul[@class='list-unstyled']//li[{AppConstant.rd.Next(1, 15)}]/a");
            Thread.Sleep(TimeSpan.FromSeconds(AppConstant.rd.Next(10, 20)));
